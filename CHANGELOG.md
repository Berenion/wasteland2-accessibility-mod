...
## [0.8.8-beta] — 2026-09-04

Reachability read-outs on the exploration cursor, conversations that no longer skip themselves, and an installer that explains a missing C++ runtime.

### Added
- **Walk-there checks on the exploration cursor** — `P` turns on a reachability read-out for the grid cursor. Each tile then also says whether the selected ranger could get there: "reachable", "no route", "nothing walkable nearby", "behind Cell Door, closed, locked, at 42, 17" when a shut door is what cut the route, or "through Sewer Grate, at 120, 88" when the thing is in a part of the level you travel to rather than walk to — the coordinates are the door's own tile, so you can send the cursor straight to it. Reaching means being able to walk up to the tile within the game's own interact range, so a container or corpse you can stand beside counts as reachable even though its own tile is not walkable. Line of sight already told you whether you could *see* a spot; this tells you whether you can *reach* it, which on the far side of a closed door is a different answer. It runs the same navmesh tests the game runs on a move click, so the verdict matches what pressing `]` would do. Also in the `Shift+S` settings menu.

### Fixed
- Pressing Enter to advance a conversation no longer skips a part of it. Two separate faults were doing this. The game often gates a whole group of lines behind a single "click to continue" and emits them a frame apart; each new line was cancelling the previous one's pending read, so every line of the group except the last went unspoken. Reads are now queued and delivered in order. Separately, Enter is bound to the game's "Attack Current Target" button, whose *release* the game treats as a click to continue — so one tap advanced twice, the second advance landing on the next line the instant it appeared and cutting off its voiceover. The release of an Enter press the mod has already acted on is now claimed rather than left for the game.
- Conversation lines the game delivers as barks are now read out. Dramas stage multi-speaker exchanges with barks rather than ordinary dialogue — asking Howdy at the Rail Nomad camp about the Desert Rangers a second time sets off an argument between him and Jill Yates — and those lines take a different route out of the game that the mod was filtering as a duplicate, so the whole exchange passed in silence. While one is playing the mod no longer calls it "loading"; Enter skips the current line instead of being swallowed.
...
- The installer now checks for the Microsoft Visual C++ 2015-2022 x64 runtime and says so when it is absent. MelonLoader's `version.dll` and the speech bridge (`Tolk.dll`, `nvdaControllerClient64.dll`) are MSVC-built natives that link the redistributable CRT; without it MelonLoader never loads, the game starts silently unmodded, and nothing in the log explains why. All three required DLLs are checked by name — a machine carrying only the 2015 package has two of the three and still fails — and the warning names the download. It warns rather than blocks: the files are still laid down, and the runtime is only needed before the next launch. The installer itself now links the CRT statically, so it still runs on the machine that needs the diagnosis.
- `installer.exe --check > log.txt` no longer writes an empty file and prints to the screen instead. Attaching a console repointed the standard handles away from the redirect; the inherited handles are now restored afterwards.
